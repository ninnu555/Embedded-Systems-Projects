...
void my_ISR (void){
    ...
    // One pression of the button will negate the state of the switches.
    // The consequent release of the button will restore the normal state of the switches.
	...
	if (*INTC_ISR & 0x1) {              // Switches interrupt (GPIO1) - sensitive to any change
		...
		*GPIO0_DATA = get_msb_bin_index_switches(GPIO1_DATA);	// driving LEDs
	}

    if (*INTC_ISR & 0x2) {              // Button interrupt (GPIO2) - sensitive to press and release
        // Button interrupt detected!
        *INTC_IAR   = 0x2;
        *GPIO2_ISR  = 0x1;
        if (*GPIO2_DATA & 0b1)          // Button pression detected!
            // Invert mode: when pushing the button, it will show the inverted binary index value of the switches
            *GPIO0_DATA = ~get_msb_bin_index_switches(GPIO1_DATA);
            else                        // Button release detected!
            // Normal mode: when releasing the button, it will show the binary index value of the switches
            *GPIO0_DATA = get_msb_bin_index_switches(GPIO1_DATA);
    }
}
