  ...
  void perform_HistEqualAlgo(int img_size, unsigned char* img) {
  // compute image histogram stretching algorithm
    int hist_size = 256;
    int hist[hist_size];
    ...
    // assigning a value to hist[]
      hist[img[j]]++;
    }

    int cdf[hist_size];
    cdf[0] = hist[0];   // initilizing first element
    for (int i = 1; i < hist_size; i++) {
    // computing cumulative distribution (CDF)
      cdf[i] = cdf[i-1] + hist[i];
    }

    int cdf_min = img_size;   // initializing first element with the bigger possible value
    for (int i = 0; i < hist_size; i++) {
    // computing cdf_min
      if (cdf_min > cdf[i] && cdf[i] > 0)
        cdf_min = cdf[i];
    }

    int map[hist_size];
    for (int i = 0; i < hist_size; i++) {
    // creating the map for mapping operation
      map[i] = (int)(((float)(cdf[i] - cdf_min) / (img_size - cdf_min)) * 255);
    }

    for (int i = 0; i < img_size; i++) {
      // applying mapping to each pixel
      img[i] = map[img[i]];   // globally returned mapped_img
  ...
  int main()
  {
    ...
    // performing the Histogram Stretching Algorithm
    perform_HistEqualAlgo(img_size, img);
