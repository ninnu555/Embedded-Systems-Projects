...
void readFromUart (int size, short int* data) {
  // receiving image/weights/biases from computer by UART
    for (int i=0; i < size; i++) {
      ...
      data16 = (data2<<8) + data1;  // composing the 16-bit value as UART only sends 8 bit at a time
...
void sendOptimizedImageUart (int size, short int* data) {
  ...
  for (int i=0; i < size; i++) {
    XUartPs_SendByte(XPAR_PS7_UART_1_BASEADDR, (DATA) ((data[i]+4)>>3));  // Q8.8 to Q4.4 conversion
  }
}
...
int main(){
  ...
  xil_printf ("Started\n");
    ...
    // Please, diascard decommenting the prints as they ruin the readings
    //////////////////////////////////////////////////////
  ...
  while (1) {
    ...
    // Please, diascard decommenting the print as they ruin the readings
    /////////////////////////////////////////////////////
    // // print("Open CAPTURE\r\n");
    ...
    first_FC_forward_time[0] = Xil_In32(GLOBAL_TMR_BASEADDR + GTIMER_COUNTER_LOWER_OFFSET);
      // we will use the 8 bit wide optimized image for the DNN (opt_img)
